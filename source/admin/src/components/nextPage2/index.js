...
export default {
  install: (Vue, option) => {
    ...
    Vue.prototype.$nextPage = (options) => {
      ...
      const userOptions = Object.assign({}, mergeOptions, options)
      //
      userOptions.currentRoute = userOptions.router.currentRoute
      // 如果配置了需要缓存,则打开的该组件将会被缓存起来.
      if (userOptions.cache) userOptions.cacheList.push(options.component.name)
      //
      userOptions.component.title = userOptions.title || undefined

      // 为自定义组件添加一个关闭当前自定义组件的方法
      if (userOptions.component.methods === undefined) userOptions.component.methods = {}
      userOptions.component.methods.closeNextPage = (component) => {
      ...
      NpInstance.nextPages.push(userOptions.component)
      ...
      NpInstance.__closeNextPage = (name) => {
        ...
        if (NpInstance.nextPages.length) {
          ...
          try {
            NpInstance.destroyNextPage.call(this)
